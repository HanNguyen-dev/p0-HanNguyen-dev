...
namespace PizzaBox.Storing.Repositories
{
  public class OrderRepository
  {
    ...
    public static void Create(Transaction transaction_1)
    {
      transactions.Add(transaction_1);
      Save();

    }
    public static void Delete(Transaction transaction_1)
    {
      Transaction orderItem = transactions.Find( item => item.orderNum == transaction_1.orderNum);
      transactions.Remove(orderItem);
      Save();
    }
    public static void Save()
    {
      OrderFileConnector.WriteXml(transactions);
    }
    public static List<Transaction> Read(Transaction transaction_1)
    {
      if (transaction_1 == null)
      {
        return transactions;
      }
      return transactions.Where( item => item.orderNum == transaction_1.orderNum).ToList();
    }
